defmodule LessAlexa.Parsers do
  ...
  @moduledoc """
  A plug for parsing the request body. This is adapted from Plug.Parsers to
  allow us to store a copy of the raw body for later access. Ideally
  we can merge this functionality into a future version of Plug itself.
  ...
  * `Plug.Parsers.JSON` - parses `application/json` requests with the given
  ...
  # This is why we need to copy this over form Plug. We need the ability to use
  # our own parser for JSON.
  defp convert_parsers(parsers) do
    for parser <- parsers do
      IO.puts parser
      case Atom.to_string(parser) do
        "Elixir." <> _ -> parser
        reference      -> Module.concat(LessAlexa.Parsers, String.upcase(reference))
      end
    end
  end
